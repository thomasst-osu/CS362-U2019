...
int playBaronCard(int choice1, int currentPlayer, struct gameState *state){
  ...
  if (choice1 > 0){
    ...
    while (card_not_discarded){
      if (state->hand[currentPlayer][p] == estate){   // estate card found
...
int playTributeCard(int currentPlayer, int nextPlayer, int tributeRevealedCards[2], struct gameState *state){
  ...
  // Determine what type of cards are revealed and execute appropriate action
  for (int i = 0; i <= 2; i++){
...
int playMineCard(int choice1, int choice2, int currentPlayer, int handPos, struct gameState *state){
  ...
  if (choice2 > treasure_map || choice2 < curse){   // requested treasure validation
    return -1;
  }

  if ((getCost(state->hand[currentPlayer][choice1]) + 3) > getCost(choice2)){   // ensure cost of requested item is allowed
    return -1;
  }

  gainCard(choice2, state, 2, currentPlayer);

  // discard current card from hand
  discardCard(handPos, currentPlayer, state, 0);
